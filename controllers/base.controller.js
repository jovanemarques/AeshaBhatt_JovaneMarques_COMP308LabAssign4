...
function BaseController() {
  ...
  this.save_date_last_update = this.options.save_date_last_update || false;
}
...
BaseController.prototype.add = function(req, res) {
  ...
  newModel.save(function(err, model) {
    if (err) return res.status(500).send(err);
    that.save_last_update(newModel.collection.name);
    res.json(model);
  });
};
...
BaseController.prototype.update = function(req, res) {
  ...
  this.model.findByIdAndUpdate({
    ...
    function(err, model) {
      if (err) return res.status(500).send(err);
      that.save_last_update(that.model.collection.name);
      res.json(model);
    });
};
